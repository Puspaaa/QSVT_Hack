...
with st.sidebar:
    ...
    # Generate n_timesteps evenly spaced time steps
...
with col1:
    st.markdown("""
    We solve the 1D advection-diffusion PDE with periodic boundary conditions:
    """)
    ...
    - $c$ is the **advection velocity** - controls translation
    - Domain: $x \\in [0, 1)$ with periodic boundaries
    """)
...
st.header("⚛️ Step 3: QSVT Circuit Structure")
st.markdown("""
...
if st.session_state.angles_computed:
    ...
    st.markdown("### ⏱️ Time Step Color Legend")
    ...
    if run_sim_btn:
        st.subheader("📊 Time Evolution: Watch the Wave Evolve")
